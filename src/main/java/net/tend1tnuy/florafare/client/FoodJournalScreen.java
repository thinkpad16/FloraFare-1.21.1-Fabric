...
public class FoodJournalScreen extends Screen {
     ...
     * or a setting the journal reads changes.
     *
     * <p>Clears the entries as well as the flag. The lists hold one ItemStack and one
     * config per food in the game, and leaving them populated meant a client that had
     * opened the journal once kept the whole of the previous world's food list alive for
     * the rest of the session, even after disconnecting.
     */
    public static void invalidateCache() {
        CACHE.valid = false;
        // Replaced, never cleared in place. The lists hold one ItemStack and one config
        // per food in the game, so leaving them populated kept the previous world's whole
        // food list alive for the rest of the session — but a screen that is open right
        // now holds a direct reference to them as its unfiltered view, and emptying that
        // underneath it would blank the grid mid-render. Handing the cache a fresh pair
        // releases the old ones while the open screen keeps rendering what it already had.
        CACHE.foods     = new ArrayList<>();
        CACHE.synergies = new ArrayList<>();
    ...
    private void rebuildCache() {
        // Fresh lists rather than clear(), for the same reason as invalidateCache().
        CACHE.foods     = new ArrayList<>();
        CACHE.synergies = new ArrayList<>();
        CACHE.unlockedFoods     = 0;
        CACHE.unlockedSynergies = 0;
        ...
        for (net.minecraft.item.Item item : Registries.ITEM) {
            ...
            // Auto-generated entries used to be skipped, so a modpack's food showed up
            // in the journal only if a datapack named it — which in practice meant the
            // journal listed a few dozen vanilla items and silently omitted everything
            // the player was actually eating. They are listed now; the toggle exists
            // for packs that curate their food list and want the journal to match it.
            if (!FlorafareConfig.journalShowsAutoGenerated
                    && !datapackTargets.contains(data.target())) {
                continue;
            }
    ...
    private void rebuildFilteredLists() {
        ...
        if (query.isEmpty()) {
            // Aliases, not copies: these two lists are only ever replaced, never mutated
            // through the filtered view, and the unfiltered case is the common one.
    ...
    private void buildDetailPages(FoodBuffData data) {
        ...
        allLines.add(new RenderLine(
                Text.translatable("gui.florafare.journal.duration",
                        BuffDescription.mmss(data.duration())).getString(),
                COLOR_INK_MID, 12, 0));
        allLines.add(new RenderLine(
                Text.translatable("gui.florafare.journal.nutrition", data.nutrition()).getString(),
                COLOR_INK_MID, 12, 0));
        // The datapack field is a saturation *modifier*; the page shows what it actually
        // restores, the same number the item tooltip and the EMI panel print.
        allLines.add(new RenderLine(
                Text.translatable("gui.florafare.journal.saturation",
                        BuffDescription.trimFloat(data.nutrition() * data.saturation() * 2.0f)).getString(),
                COLOR_INK_MID, 12, 0));
    ...
    private void buildCommonDetailLines(List<RenderLine> allLines, double healthBonus,
        ...
        if (effects != null && !effects.isEmpty()) {
            ...
            for (var effect : effects) {
                allLines.add(new RenderLine(
                        "• " + BuffDescription.effectName(effect.id()).getString()
                                + BuffDescription.amplifierNumeral(effect.amplifier())
                                + " (" + BuffDescription.mmss(effect.duration()) + ")",
                        COLOR_INK_LIGHT, 11, 4));
            }
        }
    }
        ...
        if (currentState == ScreenState.INDEX) {
            ...
            if (hoveredSynergy != null) {
                if (!hoveredSynergy.isUnlocked) {
                    context.drawTooltip(this.textRenderer,
                            BuffDescription.lockedLines(), mouseX, mouseY);
                } else {
                    context.drawTooltip(this.textRenderer,
    ...
    /**
     * Scroll wheel turns pages, in whichever view is showing. A book with page arrows
     * and no scroll support reads as broken to anyone who has used one of the big
     * recipe-book mods, and the gesture is free here because the journal never has a
     * scrollable region of its own competing for it.
     */
    @Override
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontal, double vertical) {
        ...
        if (currentState == ScreenState.FOOD_DETAIL || currentState == ScreenState.SYNERGY_DETAIL) {
            int target = detailCurrentPage + (forward ? 1 : -1);
            if (target < 0 || target >= detailPages.size()) return true;
            detailCurrentPage = target;
        } else if (currentState == ScreenState.FOOD_GRID || currentState == ScreenState.SYNERGY_GRID) {
            int target = currentPage + (forward ? 1 : -1);
            if (target < 0 || target >= maxPages) return true;
            currentPage = target;
        } else {
            return super.mouseScrolled(mouseX, mouseY, horizontal, vertical);
        }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (searchField != null && searchField.isVisible() && !searchField.getText().isEmpty()) {
            searchField.setText("");
            return true;
        }
