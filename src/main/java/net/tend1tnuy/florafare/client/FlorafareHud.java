...
public class FlorafareHud implements HudRenderCallback {
    ...
    private void renderActiveSlot(DrawContext context, MinecraftClient client, int index, ActiveFoodBuff buff, int currentX, float fadeAlpha) {
        ...
        float progress = Math.min(1.0f, Math.max(0.0f, remainingTicks / initialDuration));
        ...
        int textColorWhite = (alphaInt << 24) | 0xFFFFFF;
        int textColorGray = (alphaInt << 24) | 0xAAAAAA;

        context.fill(currentX, y, currentX + SLOT_WIDTH, y + SLOT_HEIGHT, bgColor);
        context.drawBorder(currentX, y, SLOT_WIDTH, SLOT_HEIGHT, borderColor);

        int barWidth = (int) ((SLOT_WIDTH - 2) * progress);
        if (barWidth > 0) {
            int colorTop = isBlinking ? 0xFFFF5555 : 0xFF55FF55;
            int colorBottom = isBlinking ? 0xFFAA0000 : 0xFF00AA00;

            colorTop = (colorTop & 0x00FFFFFF) | (alphaInt << 24);
            colorBottom = (colorBottom & 0x00FFFFFF) | (alphaInt << 24);

            int barY = y + 17;
            int barHeight = 4;

            context.fill(currentX + 1, barY, currentX + 1 + barWidth, barY + barHeight / 2, colorTop);
            context.fill(currentX + 1, barY + barHeight / 2, currentX + 1 + barWidth, barY + barHeight, colorBottom);
        }
        ...
        context.drawTextWithShadow(client.textRenderer, timeStr, currentX + 22, y + 6, textColorWhite);

        String name = stack.getName().getString();
        if (client.textRenderer.getWidth(name) > 65) {
            name = client.textRenderer.trimToWidth(name, 60) + "...";
        }
        context.drawTextWithShadow(client.textRenderer, name, currentX + 55, y + 6, textColorGray);
    }
    ...
    private void renderEmptySlot(DrawContext context, MinecraftClient client, int index) {
        ...
        context.drawTextWithShadow(client.textRenderer, "Empty", BASE_X + 22, y + 6, 0x44FFFFFF);
    }
}